...
def load_data():
    left_filename = "fma_albums_full.json"
    right_filename = "fma_albums_learned_feature.json"
    with open(left_filename, "r") as infile:
        raw_left_data = json.load(infile)
    logging.info("successfully parsed {}".format(left_filename))
    with open(right_filename, "r") as infile:
        raw_right_data = json.load(infile)
    logging.info("successfully parsed {}".format(right_filename))
    training, validation, test = list(), list(), list()
    stats = {"min_track_number": float("inf"), "max_track_number": 0}
    while raw_left_data:
        raw_left_album = raw_left_data.pop()
        raw_right_album = raw_right_data.pop()
        for i in range(len(raw_left_album)):
            raw_left_album[i].update(raw_right_album[i])
        raw_album = raw_left_album
        min_album_track, max_album_track = float("inf"), 0
        album = list()
        for raw_track in raw_album:
            album.append(
                (raw_track["track number"], raw_track["learned scalar feature"])
            )
...
def fitness(tpls, dataset):
    ...
    for album in dataset:
        ...
        for tpl in expanded_tpls:
            error = 0
            for track_number, value in album:
                error += np.square(tpl(track_number) - value)
            error /= len(album)
            if error < best:
                best = error
        rv += best
